...
from ppuda.utils import AvgrageMeter, adjust_net, infer
from ppuda.vision.loader import image_loader

from ghn3 import DeepNets1MDDP, from_pretrained, get_metadata
from ghn3.ops import Network

parser = argparse.ArgumentParser(description="Evaluation of GHNs")
parser.add_argument(
    "--save_ckpt", type=str, default=None, help="checkpoint path to save the model with predicted parameters"
)
...
images_val, num_classes = image_loader(
    args.dataset,
    args.data_dir,
    test=True,
...
assert ghn.num_classes == num_classes, (
    "The eval image dataset and the dataset the GHN was trained on must match, "
...
if is_torch:
    ...
    models_queue = DeepNets1MDDP.loader(
        meta_batch_size=1,
        dense=ghn.is_dense(),
        split=args.split,
...
for m_ind, m in enumerate(models_queue):
    try:
        ...
        if is_torch or (not is_torch and isinstance(m.net_args[0]["genotype"], str)):
            ...
            model = Network(is_imagenet_input=is_imagenet, num_classes=num_classes, **m[0].net_args)
            graphs = m
            m = str(m[0].net_idx)
        ...
        print("\n{}/{}: {} with {:.2f}M parameters".format(m_ind + 1, len(models_queue), m.upper(), n_params), end="...")
        if args.device != "cpu":
...
print(
    "\nresults: count={} (avg\u00B1std) top1={:.3f}\u00B1{:.3f}".format(top1_all.cnt, top1_all.avg, top1_all.dispersion)
)
