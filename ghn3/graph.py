...
from ppuda.deepnets1m.net import Network as NetworkPPUDA
...
class GraphBatch:
    ...
    def append(self, graph):
        ...
        if self.dense:
            ...
            self.node_feat.append(
                torch.cat(
                    (
                        graph.node_feat,  # primitive type
            ...
            self.edges.append(
                torch.cat((graph.edges, graph_offset + torch.zeros(len(graph.edges), 1, dtype=torch.long)), dim=1)
...
class Graph:
    ...
    def __init__(
        ...
        net_idx=None,
        ve_cutoff=50,
        list_all_nodes=False,
        reduce_graph=True,
        fix_weight_edges=True,
        fix_softmax_edges=True,
        dense=False,
        verbose=True,
    ):
        r"""
        ...
        if model is not None:
            ...
            sz = (
                model.expected_input_sz
                if hasattr(model, "expected_input_sz")
                else (299 if isinstance(model, torchvision.models.Inception3) else 224)
            )
    ...
    def _build_graph(self):
        ...
        This function is written based on Sergey Zagoruyko's
        https://github.com/szagoruyko/pytorchviz/blob/master/torchviz/dot.py (MIT License)
        ...
        def traverse_graph(fn):
            ...
            if fn_name.find("AccumulateGrad") < 0:
                ...
                if len(leaf_nodes) == 0:
                    leaf_nodes.append({"id": fn, "param_name": fn_name, "attrs": get_attr(fn), "module": None})
            ...
            for i, node in enumerate(nodes):
                if node["param_name"].lower().endswith("norm.weight"):
                    for out_neigh in np.where(A[i, :])[0]:
                        if (
                            nodes[out_neigh]["param_name"].endswith("norm1.weight")
                            or nodes[out_neigh]["param_name"].find("Add") >= 0
                        ):
                            A[i, out_neigh] = 0
            ...
            if isinstance(self.model, (models.VisionTransformer, Network, NetworkPPUDA)):
                ...
                for i, node in enumerate(nodes):
                    if isinstance(node["module"], (PosEnc, ppuda_ops.PosEnc, models.vision_transformer.Encoder)):
                        nodes.insert(
                            i + 1, {"id": "sum_pos_enc", "param_name": "AddBackward0", "attrs": None, "module": None}
                        )
    ...
    def _filter_graph(self, unsupported_modules=None):
        ...
        for module_name in unsupported_modules:
            ...
            for i, node in enumerate(self._nodes):
                ...
                op_name, _ = node["param_name"], node["attrs"]
                ...
                if op_name.find(module_name) >= 0:
                    ...
                    if op_name.startswith("Mean"):
                        ...
                        keep = (
                            has_sigmoid_swish_cse
                            and not classifier_head
                            and (
                                neighbors[-2].startswith(("hard", "sigmoid"))
                                or neighbors[-3].startswith(("relu", "mean"))
                                or neighbors[1].startswith(("hard", "sigmoid", "relu"))
                            )
                        )
    ...
    def _add_virtual_edges(self, ve_cutoff=50):
        ...
        assert self._Adj[np.diag_indices_from(self._Adj)].sum() == 0, (
            "no loops should be in the graph",
            self._Adj[np.diag_indices_from(self._Adj)].sum(),
        )
        ...
        if self._verbose:
            ...
            for node in range(self.n_nodes):
                if node not in length and not self._nodes[node]["param_name"].lower().startswith("aux"):
                    print(
                        "WARNING: node={}-{} does not have a path to node={}-{}".format(
            ...
            for node in range(self.n_nodes):
                ...
                if not (
                    target_name.startswith("pos_enc")
                    or target_name.find("pos_emb") >= 0
                    or target_name.find("position_bias") >= 0
                    or source_name.find("position_bias") >= 0
                ):
                    print(
                        "WARNING: node={}-{} does not have a path to node={}-{}".format(
    ...
    def _named_modules(self):
        ...
        for n, m in self.model.named_modules():
            for np_, p in m.named_parameters(recurse=False):
                ...
                key = n + "." + np_
                if key in modules:
                    assert id(p) == id(modules[key][0]), (n, np_, p.shape, modules[key][0].shape)
                    continue
                modules[key] = (p, m)
        ...
        if len(modules) > n_tensors:
            if self._verbose:
                print(
                    "WARNING: number of tensors found in all submodules ({}) > number of unique tensors ({}). "
    ...
    def visualize(
        ...
        **nx_args,
        ...
        def color(c):
            return cm.jet(int(np.round(255 * c / n_primitives)))
        ...
        node_groups = {
            "bn": {"style": {"edgecolors": "k", "linewidths": 1, "node_shape": "s"}},
        ...
        if vis_legend:
            node_feat = torch.cat((torch.tensor([n_primitives]).view(-1, 1), torch.tensor(primitives_ord)[:, None]))
        ...
        for i, (x, sz) in enumerate(zip(node_feat.view(-1), param_shapes)):
            ...
            if detailed_labels:
                labels[i] = (
                    self._nodes[i]["param_name"].replace("features", "f").replace(".weight", ".w").replace(".bias", ".b")
                )
            else:
