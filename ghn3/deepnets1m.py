...
from ppuda.deepnets1m.genotypes import PRIMITIVES_DEEPNETS1M, from_dict
from ppuda.deepnets1m.loader import MAX_NODES_BATCH, DeepNets1M, NetBatchSampler
...
class DeepNets1MDDP(DeepNets1M):
    ...
    def __init__(self, dense=True, wider_nets=True, debug=False, **kwargs):
        ...
        if self.split != "predefined":
            ...
            assert len(self.primitives_dict) == len(PRIMITIVES_DEEPNETS1M), (
                len(self.primitives_dict),
    ...
    def __getitem__(self, idx):
        if self.split == "predefined":
            ...
            if self.is_train:
                is_conv_dense = sum([n[0] in ["conv_5x5", "conv_7x7"] for n in cell.normal + cell.reduce]) > 0
                ...
                num_params = (
                    args["num_params"]["imagenet" if self.large_images and not self.wider_nets else "cifar10"] / 10**6
                )

                # attempt to use a smaller stride for imagenet to reduce the bias to stride = 4
                if (
                    self.wider_nets
                    and self.large_images
                    and args["glob_avg"]
                    and args["stem_type"] == 0
                    and args["stem_pool"]
                    and not (num_params > 0.2 or n_cells > 8 or is_conv_dense)
                ):
                ...
                if (
                    num_params > (2.0 if self.wider_nets else 0.8)
                    or not args["glob_avg"]
                    or is_conv_dense
                    or n_cells > (14 if self.wider_nets else 12)
                ):
            ...
            graph = self._init_graph(
                self.h5_data[self.split][str(idx)]["adj"][()], self.h5_data[self.split][str(idx)]["nodes"][()], net_args
            )
            graph.net_idx = idx
            if self.is_train and not self.debug:
                graph.net = NetworkLight(
                    is_imagenet_input=self.large_images, num_classes=1000 if self.large_images else 10, **net_args
                )
    ...
    def _init_graph(self, A, nodes, net_args):
            ...
            if net_args["norm"] is not None:
                ...
                if self.debug:
                    assert self.op_names_net[nodes[stem0][2]] == "stem0.4.weight", (
                        self.op_names_net[nodes[stem0][2]],
                        net_args,
                    )
                    assert self.op_names_net[nodes[stem1][2]] == "stem1.2.weight", (
                        self.op_names_net[nodes[stem1][2]],
                        net_args,
                    )
            else:
                stem0, stem1 = 2, 3
        ...
        for node_ind, node in enumerate(nodes):
            ...
            if name.find("conv") >= 0 or name.find("pool") >= 0 or name in ["bias", "bn", "ln", "pos_enc"]:
                node_info[cell_ind].append(
                    (node_ind, name_op_net, name, sz, node_ind == len(nodes) - 2, node_ind == len(nodes) - 1)
                )
...
class NetBatchSamplerDDP(NetBatchSampler):
    ...
    def __init__(self, deepnets, meta_batch_size=1):
        ...
        self.max_nodes_batch = (
            int(MAX_NODES_BATCH / 8 * max(8, meta_batch_size)) if deepnets.is_train and meta_batch_size > 1 else None
        )
        log("max_nodes_batch", self.max_nodes_batch, "meta_batch_size", meta_batch_size)

    def check_batch(self, batch):
        return (
            self.max_nodes_batch is None
            or (self.sampler.dataset if is_ddp() else self.sampler.data_source).nodes[batch].sum()
            <= self.max_nodes_batch
        )
